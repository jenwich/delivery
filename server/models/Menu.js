...
function loadOne(menu_id, callback) {
    query(LOAD_ONE, [menu_id], function(err, rows) {
        callback(err, rows[0]);
    });
}
...
function loadCategoryByStore(store_id, callback) {
    query(LOAD_CATEGORY_BY_STORE, [store_id], function (err, rows) {
        if (rows.length) {
...
function getStoreOfMenus(menu_arr, callback) {
    query(LOAD_STORES, [[menu_arr]], function(err, rows) {
...
function getPriceOfMenus(menu_arr, callback) {
    query(LOAD_PRICE, [[menu_arr]], function(err, rows) {
...
function getNameOfMenus(menu_arr, callback) {
    query(LOAD_NAME, [[menu_arr]], function(err, rows) {
...
function changeAvailable(menu_id, value, callback) {
    query(CHANGE_AVAILABLE, [value, menu_id], callback);
}
